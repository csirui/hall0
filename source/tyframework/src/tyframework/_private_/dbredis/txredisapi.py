...
# args[0]=src_tasklet_id, args[1]=event
def txlog(*args, **kwargs):
    taskid = args[0]
    event = args[1]
    try:
        tstamp = time.time() - 1415680000
        # print tstamp, taskid, stackless.getcurrent()._task_id, event, args[2:]
        if taskid == -1:
            return
        if taskid in GLOBAL_TASK_MAP:
            datas = GLOBAL_TASK_MAP[taskid]
            datas.append((event, tstamp))
            if event == "PUTBACK":
                if datas[-1][1] - datas[1][1] > 0.8:
                    print "TASK", taskid, datas
                del GLOBAL_TASK_MAP[taskid]
        else:
            GLOBAL_TASK_MAP[taskid] = [' '.join([unicode(x) for x in args[2:]])]
            GLOBAL_TASK_MAP[taskid].append((event, tstamp))
    except Exception, e:
        if taskid in GLOBAL_TASK_MAP:
            del GLOBAL_TASK_MAP[taskid]
        print 'txlog-error', e
...
class RedisProtocol(LineReceiver, policies.TimeoutMixin):
    ...
    def replyReceived(self, reply):
        ...
        function.
        """
        txlog(self.srctasklet, "REDIS-REPLY")
        self.replyQueue.put(reply)

    @staticmethod
    def handle_reply(r, _src):
        txlog(_src, "HANDLE-REPLY")
        # print time.time()-1415669382,  stackless.getcurrent()._task_id, "HANDLEREPLY"
    ...
    def execute_command(self, *args, **kwargs):
        if self.connected == 0:
            ...
            txlog(_srctask, "WRITEREDIS")
            r = self.replyQueue.get().addCallback(self.handle_reply, _srctask)
    ...
    def __getattr__(self, method):
        def wrapper(*args, **kwargs):
            if "taskid" in kwargs:
                ...
                _srctask = -1
            txlog(_srctask, "GETATTR", args, kwargs)
            d = self._factory.getConnection()
            txlog(_srctask, "AFTER-FACTORY-GETCONN", d)

            def callback(connection):
                connection.srctasklet = _srctask
                txlog(_srctask, "FACTORY-GETCONN-OK", connection)
                protocol_method = getattr(connection, method)
                try:
                    txlog(_srctask, "BEFORE-PROTOCOL-METHOD", connection)
                    d = protocol_method(*args, **kwargs)
                    txlog(_srctask, "AFTER-PROTOCOL-METHOD", connection)
                except:
                    self._factory.connectionQueue.put(connection)
                    raise

                def put_back(reply):
                    txlog(_srctask, "PUTBACK")
                ...
                txlog(_srctask, "WRAPPER_CALLBACK-RETURN", d)
                return d

            d.addCallback(callback)
            txlog(_srctask, "WRAPPER-RETURN", d)
            return d
