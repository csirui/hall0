...
class TyCodec(object):

    ffi_out_str = ffi.new("unsigned char[]", 65536)

    @classmethod
    def __des_encrypt(cls, deskey, desstr):
        deslen = len(desstr)
        if deslen > 65000 :
            raise Exception('the desstr length too long !! 65000 limited !!')
        outlen = C.des_encrypt(desstr, deslen, deskey, cls.ffi_out_str)
        return ffi.buffer(cls.ffi_out_str, outlen)[:]

    @classmethod
    def __des_decrypt(cls, deskey, desstr):
        deslen = len(desstr)
        if deslen > 65000 :
            raise Exception('the desstr length too long !! 65000 limited !!')
        outlen = C.des_decrypt(desstr, deslen, deskey, cls.ffi_out_str)
        return ffi.buffer(cls.ffi_out_str, outlen)[:]

    @classmethod
    def des_encode(cls, desstr):
        desstr = 'TUYOO~' + desstr + '~POKER201309031548'
        desstr = cls.__des_encrypt('tuyoocom', desstr)
        desstr = base64.b64encode(desstr)
        return desstr

    @classmethod
    def des_decode(cls, desstr):
        desstr = base64.b64decode(desstr)
        desstr = cls.__des_decrypt('tuyoocom', desstr)
