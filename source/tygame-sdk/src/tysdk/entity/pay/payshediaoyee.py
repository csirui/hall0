...
class TuYouPayShediaoYee():
    # 商户编号p1_MerId,以及密钥merchantKey 需要从易宝支付平台获得
    p1_MerId = "10012424968"
    merchantKey = "9PQD38l00eT3CRNbH9t0p92i42gti14o35309dR380C28W86HD3d00C9UrtS"
    # 非银行卡支付专业版请求地址,无需更改.
    reqURL_SNDApro = "https://www.yeepay.com/app-merchant-proxy/command.action?"
    # 易宝回调的地址
    callback_url = "/v1/pay/shediaoyee/callback"
    ...
    def getHmacString(cls, hstr):
        ...
        mac = hmac.new(TuYouPayShediaoYee.merchantKey)
        mac.update(sbOld)
    ...
    def annulCard(self, datas, rparam):
        TyContext.ftlog.info('TuYouPayShediaoYee->annulCard->rparam=', rparam, 'datas=', datas)
        ...
        TyContext.ftlog.debug('TuYouPayShediaoYee->annulCard->hmacstr=', hmacstr)
        # 进行加密串处理，一定按照下列顺序进行
        httpdomain = PayHelper.getSdkDomain()
        params = {
            ...
            'p8_Url': httpdomain + TuYouPayShediaoYee.callback_url,  # 加入商户接收交易结果通知的地址
        ...
        payUrl = TuYouPayShediaoYee.reqURL_SNDApro + TuYouPayShediaoYee.createQueryString(params)
        TyContext.ftlog.info('TuYouPayShediaoYee->annulCard->payUrl=', payUrl)
        response, payUrl = TyContext.WebPage.webget(payUrl)
        TyContext.ftlog.info('TuYouPayShediaoYee->annulCard->payUrl=', payUrl, 'response=', response)
    ...
    @classmethod
    def doPayRequestCard(cls, datas):
        userId = datas['userId']
        uct = TyContext.RedisUser.execute(userId, 'HGET', 'user:' + str(userId), 'createTime')
        params = {
            'p2_Order': datas['orderPlatformId'],
            'p3_Amt': str(int(float(datas['orderPrice']))),
            'p4_verifyAmt': 'true',
            'p5_Pid': '',
            'p6_Pcat': '',
            'p7_Pdesc': '',
            'pa_MP': '',
            'pa7_cardAmt': datas['card_amount'],  # 多卡号以逗号隔开
            'pa8_cardNo': datas['card_number'],  # 多卡号以逗号隔开
            'pa9_cardPwd': datas['card_pwd'],  # 多卡号以逗号隔开
            'pd_FrpId': datas['card_code'],  # 支付卡的通道编码
            'pz_userId': str(userId),
            'pz1_userRegTime': str(uct)
        }
        mo = cls.annulCard(datas, params)
        return mo
    ...
    def doCardCallback(cls, rpath):
        ...
        if iHmacStr != cHmacStr:
            TyContext.ftlog.error('TuYouPayShediaoYee.doCardCallback, hmac verification error')
            return 'error'
        rparam = TyContext.RunHttp.convertArgsToDict()

        mer_code = TyContext.RunHttp.getRequestParam('p1_MerId')
        if mer_code != TuYouPayShediaoYee.p1_MerId:
            TyContext.ftlog.error('TuYouPayShediaoYee.doCardCallback error, mer_code is not me !!!')
            return 'error'
        ...
        if r1_Code != '1':
            ...
            TyContext.ftlog.error('TuYouPayShediaoYee.doCardCallback error, charge return error !!! status=', status)
            bank_pay_flag = u'支付失败，' + TuYouPayShediaoYee.code_map.get(status, 'status' + str(status))
