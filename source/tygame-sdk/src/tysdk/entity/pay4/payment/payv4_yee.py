...
class TuYouPayYeeV4(PayBaseV4):
    ...
    def getHmacString(cls, hstr):
        ...
        mac = hmac.new(TuYouPayYeeV4.merchantKey)
        mac.update(sbOld)
    ...
    def getReqHmacString(self, rparam, p0_Cmd, pr_NeedResponse):
        ...
        sbOld += httpdomain + TuYouPayYeeV4.callback_url
        # 加入临时信息
        sbOld += rparam['pa_MP']
        # 加入卡面额组
    ...
    def annulCard(self, chargeInfo, mi, mo, rparam):
        ...
        params = {
            ...
            'p1_MerId': TuYouPayYeeV4.p1_MerId,  # 加入商家ID
            ...
            'p8_Url': httpdomain + TuYouPayYeeV4.callback_url,  # 加入商户接收交易结果通知的地址
        ...
        payUrl = TuYouPayYeeV4.reqURL_SNDApro + TuYouPayYeeV4.createQueryString(params)
        ...
        return TuYouPayYeeV4.getPayResult(response, chargeInfo, mo)
    ...
    @payv4_callback('/open/ve/pay/yee/callback')
    def doCardCallback(cls, rpath):
        ...
        if mer_code != TuYouPayYeeV4.p1_MerId:
        ...
        if r1_Code != '1':
            ...
            errorInfo = u'支付失败，' + TuYouPayYeeV4.code_map.get(status, 'status' + str(status))
