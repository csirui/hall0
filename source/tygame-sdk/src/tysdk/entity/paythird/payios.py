...
class TuYouPayIos(object):
                    ...
                    if user_uuid != None:
                        ...
                        if checkret:
                            ...
                            if ios_uuid_pay_total_open:
                                return 'uuid_prohibit'
                        # 更新充值uuid的次数及金额
                        if 'chargeTotal' in chargeInfo and chargeInfo['chargeTotal']:
    ...
    def _deliver_missing_order(cls, rparam):
        ...
        appInfo = TyContext.RunHttp.getRequestParam('appInfo', '')
        from paytuyooios import TuYouPayMyIos
        product = TuYouPayMyIos.get_pay_ios_product(appId, iosId, clientId, 'iosid')
        if not product:
            TyContext.ftlog.error('get_pay_ios_product products not found!'
    ...
    @classmethod
    def _check_uuid_ios_pay(cls, uuid, userId, appId):
        if not uuid:
