...
class TuYouPayHuiYuanBaoYue(object):
    @classmethod
    def charge_data(cls, chargeinfo):
        ...
        if 0 == cmp(phoneType, 'chinaMobile'):
            ...
            if citycode == 1:
                cls.chargeDataErrorMsg(chargeinfo, shortOrderId, citycode)
                return
            elif citycode == 2:
                TyContext.ftlog.info(
                    'TuYouPayHuiYuanBaoYue->charge_data User client not support Monthly VIP, iccid: [%s]' % iccid)
                cls.reportBi(Order.CLIENT_CANCELED, reportBiParams)
                cls.chargeDataErrorMsg(chargeinfo, shortOrderId, citycode)
                return
            ...
            if not smsInfo:
                cls.chargeDataErrorMsg(chargeinfo, shortOrderId, 2)
                cls.reportBi(Order.CLIENT_CANCELED, reportBiParams)
                TyContext.ftlog.info(
                    'TuYouPayHuiYuanBaoYue->charge_data User client not support Monthly VIP, iccid: [%s]' % iccid)
                return
            ...
            TyContext.RedisPayData.execute('SET', 'unsubMonthlySmsInfo:' + str(userId),
                                           cls.getUnsubscribeSms(unsub_sms_msg, unsub_spnumber))
            ...
            cls.chargeDataErrorMsg(chargeinfo, shortOrderId, 1)
            return
    ...
    def notifyGameServerDelivery(cls, rparam):
        ...
        cls.changeUserInfoStatus('vipuser:' + rparam['mobile'], 'success')
        TyContext.ftlog.info(
