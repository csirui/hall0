...
class PayHelper():
    ...
    def callback_ok(cls, platformOrderId, total_fee, rparam):
        from tysdk.entity.pay3.callback import TuyouPayCallBack
        total_fee = float(total_fee)
        TyContext.RunMode.get_server_link(platformOrderId)
        ret = TuyouPayCallBack.callback(platformOrderId, total_fee, PayConst.CHARGE_STATE_CALLBACK_OK, rparam, '')
    ...
    def callback_error(cls, platformOrderId, errorInfo, rparam):
        from tysdk.entity.pay3.callback import TuyouPayCallBack
        TyContext.RunMode.get_server_link(platformOrderId)
        ret = TuyouPayCallBack.callback(platformOrderId, 0, PayConst.CHARGE_STATE_ERROR_CALLBACK, rparam, errorInfo)
        TyContext.RunMode.del_server_link(platformOrderId)
        return ret
