...
class AccountInfo():
    ...
    def doSetUserInfo(cls, rparams, mo, needReturn=True):
        ...
        if len(nickname) > 0:
            ...
            if nickname != None:
                ...
                UsernameGenerator.change_old_user_name(nickname, userId, mo_tmp)
                ...
                accessAppId = TyContext.Configure.get_global_item_json('change_nickname_appId', [12, 15])
                appId = int(rparams.get('appId', '0'))
                if not appId in accessAppId and oldname != nickname and needReturn:
                    TyContext.RedisUser.execute(userId, 'HINCRBY', 'user:' + str(userId), 'set_name_sum', 1)
    ...
    def doSetUserNickName(cls, rparams, mo, needReturn=True):
        TyContext.ftlog.info(cls.__name__, 'doSetUserNickName->rparams=', rparams, 'needReturn=', needReturn)
        userId = rparams.get('userId', 0)
        if userId <= 0:
            return
    ...
    def doSetPasswd(cls, params, mo, checkOldPasswd=True):
        ...
        TyContext.RedisUser.execute(userId, 'HMSET', 'user:' + str(userId),
                                    'password', newpasswd,
                                    'userSignature', AccountVerify.md5(newpasswd),
    ...
    def _is_clientId_forbindden_regist(cls, params):
        ...
        clientIdList = TyContext.Configure.get_game_item_json(appId, "forbiddenClientIds", {})
        if clientId in clientIdList.get('clientIds', {}):
            # POP未成功创建的snsId
            deviceId = cls.__get_param__(params, 'deviceId')
            TyContext.RedisUserKeys.execute('LPOP', 'devidtosnsidmap:' + str(deviceId))
    ...
    def createNewUser(cls, params, userType, setDevMap=True):
        ...
        clientId = cls.__get_param__(params, 'clientId')
        cls._is_clientId_forbindden_regist(params)
        ...
        datas = {
            ...
            "agreeAddFriend": 1,
            "email": "",
            '360.vip': cls.__get_param__(params, 'vip360', 0),
        }
        if datas['sex'] == '':
            datas['sex'] = '0'
        ...
        # todo for 104 test add by tcz
        datas['name'] = UsernameGenerator.generate_unique_new_user_name(uid)
        ...
        try:
            ReportIOS().on_user_create(int(uid), params)
        except:
            TyContext.ftlog.exception("ReportIOS().on_user_create error", uid)
        return int(uid)
    ...
    def fillUserLoginInfoV3(cls, params, mo, userId, isLogin, isCreate):
        ...
        bindMobile, userEmail, userSnsId, userDBPwd, changePwdCount, log_report, exception_report, user360Vip = TyContext.RedisUser.execute(
            ...
            'log_report', 'exception_report', '360.vip')
        # set default password
        if not userDBPwd:
            userDBPwd = 'ty' + str(random.randint(100000, 999999))
            TyContext.RedisUser.execute(userId, 'HMSET', 'user:' + str(userId),
                                        'password', userDBPwd,
                                        'userSignature', AccountVerify.md5(userDBPwd))
    ...
    def __append_logclient_infos__(cls, userId, clientId, mo):
        # 打开所有渠道的实时日志开关
        is_client_log_report = TyContext.Configure.get_global_item_int('client.log.report.confirm', 1)
        if is_client_log_report:
            # 根据clientid版本控制上传日志
            client_log_report_version = TyContext.Configure.get_global_item_json('client.log.report.list',
                                                                                 ['Android_3.73'])
            clientidMatch = ''
            try:
                datas = clientId.split('_')
                clientSystem = str(datas[0])
                clientVersion = str(TyContext.ClientUtils.getVersionFromClientId(clientId))
                clientidMatch = clientSystem + '_' + clientVersion
            except:
                clientidMatch = ''
            if clientidMatch != '' and clientidMatch in client_log_report_version:
                logclientDatas = {'logreporturl': 'http://client-log.tuyoogame.com/clog/report', 'loguploadurl': ''}
            else:
                logclientDatas = {'logreporturl': '', 'loguploadurl': ''}
        else:
            logclientDatas = {'logreporturl': '', 'loguploadurl': ''}
            '''
            try:
                logclientDatas = TyContext.RedisMix.execute('HGET', 'client.log.report.user.url', userId)
                if logclientDatas == None :
                    logclientDatas = TyContext.RedisMix.execute('HGET', 'client.log.report.client.url', clientId)
                if logclientDatas == None :
                    logclientDatas = TyContext.RedisMix.execute('HGET', 'client.log.report.client.url', 'default')
                if logclientDatas != None :
                    logclientDatas = TyContext.strutil.loads(logclientDatas)
                else:
                    logclientDatas = {'logreporturl': '', 'loguploadurl' : ''}
            except:
                ...
                logclientDatas = {'logreporturl': '', 'loguploadurl' : ''}
            '''
        mo.setResult('logclient', logclientDatas)
        pass
